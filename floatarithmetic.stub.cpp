...
float deserialize_float(string val) {
    return stof(val.c_str()); // note: val has a "#" at the end, but atoi ignores any non-integers
}
...
void __add(float x, float y) {
  float res = add(x, y);
  string res_str = serialize_float(res) + "#";
...
struct arith_func_info deserialize(string msg) {
  ...
  afi.arg1 = deserialize_float(arg_vals[0]);
  afi.arg2 = deserialize_float(arg_vals[1]);
  return afi;
}
